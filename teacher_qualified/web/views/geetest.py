...
class GeetestLib(object):
    ...
    def _register(self, user_id=None, new_captcha=1, JSON_FORMAT=1, client_type="web", ip_address=""):
        ...
        if pri_responce:
            ...
            challenge = " "
        if len(challenge) == 32:
            challenge = _md5_encode("".join([challenge, self.private_key]))
            return 1, challenge
        else:
    ...
    def _make_fail_challenge(self):
        ...
        md5_str1 = _md5_encode(str(rnd1))
        md5_str2 = _md5_encode(str(rnd2))
    ...
    def _register_challenge(self, user_id=None, new_captcha=1, JSON_FORMAT=1, client_type="web", ip_address=""):
        if user_id:
            register_url = "{api_url}{handler}?gt={captcha_ID}&user_id={user_id}&json_format={JSON_FORMAT}&client" \
                           "_type={client_type}&ip_address={ip_address}".format(
            ...
            register_url = "{api_url}{handler}?gt={captcha_ID}&json_format={JSON_FORMAT}&client_type={client_type}&ip" \
                           "_address={ip_address}".format(
    ...
    def success_validate(self, challenge, validate, seccode, user_id=None, gt=None, data='', userinfo='',
        ...
        if backinfo == _md5_encode(seccode):
            return 1
        else:
            return 0
    ...
    def _check_result(self, origin, validate):
        encodeStr = _md5_encode(self.private_key + "geetest" + origin)
    ...
    def _failback_check_result(self, challenge, validate):
        encodeStr = _md5_encode(challenge)
        if validate == encodeStr:
            return True
        else:
            return False
